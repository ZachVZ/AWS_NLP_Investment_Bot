...
def parse_str(n):
    """
    Securely converts a non-integer value to str.
    """
    try:
        return str(n)
    except ValueError:
...
def recommend_portfolio(intent_request):
    ...
    if source == "DialogCodeHook":
        ...
        # the elicit Slot dialog action is used to re-prompt for the first violation detected.
        if not validation_result["isValid"]:
            slots[validation_result["violatedSlot"]] = None  # Cleans invalid slot

            # Returns an elicit Slot dialog to request new data for the invalid slot
            return elicit_slot(
    ...
    content = "Thank you, " + first_name + ", for your information. "
    if risk_level == "none":
...
def validate_data(age, investment_amount, risk_level ):
    ...
    # Validate the age is between 0 and 65
    if age is not None:
        age = parse_int(
            age
        ...
        if age <= 0 or age >= 65:
            return build_validation_result(
                False,
                "age",
                "The age should be greater than 0 and less than 65. "
                "please provide a correct age.",
            )

    # Validate the investment amount, should be equal or greater than 5000.
    if investment_amount is not None:
        ...
        if investment_amount <= 5000 :
            return build_validation_result(
                False,
                "investmentAmount",
                "The investment amount should be equal or greater than 5000, "
                "please provide a correct amount.",
            )

    # Validate the risk_level, it should be the following keywords: "none", "low", "medium", "high".
    if risk_level is not None:
        ...
        if risk_level not in ["none", "low", "medium", "high"] :
            return build_validation_result(
                False,
                "riskLevel",
                "risk level should be one of the foloowing keywords (case insensitive) : \"None\", \"Low\", \"Medium\", \"High\". "
                "please provide a risk_level.",
            )
